...
class InsureePhoto(core_models.VersionedModel):
    ...
    uuid = models.CharField(
        db_column="PhotoUUID", max_length=36, default=uuid.uuid4, unique=True
    )
    ...
    folder = models.CharField(
        db_column="PhotoFolder", max_length=255, blank=True, null=True
    )
    ...
    def full_file_path(self):
        if not InsureeConfig.insuree_photos_root_path or not self.filename:
            return None
        return os.path.join(
            InsureeConfig.insuree_photos_root_path, self.folder, self.filename
        )
...
class Family(core_models.VersionedModel, core_models.ExtendableModel):
    ...
    )  # ADDED default CHECK IF YOU WANT THE BEHAVIOR
...
class Insuree(core_models.VersionedModel, core_models.ExtendableModel):
    ...
    place_of_birth = models.CharField(
        db_column="PlaceOfBirth", max_length=100, blank=True, null=True
    )
    registration_date = models.CharField(
        db_column="RegistrationDate", max_length=100, blank=True, null=True
    )
    certificate_number = models.CharField(
        db_column="CertificateNumber", max_length=100, blank=True, null=True
    )
    ...
    residential_alley = models.CharField(
        db_column="ResidentialAlley", max_length=100, blank=True, null=True
    )
    is_local = models.CharField(
        db_column="IsLocal", max_length=100, blank=True, null=True
    )
    usual_residence = models.CharField(
        db_column="UsualResidence", max_length=100, blank=True, null=True
    )
    # might be the same as profession
    occupation = models.CharField(
        db_column="Occupation", max_length=100, blank=True, null=True
    )
    father_name = models.CharField(
        db_column="FatherName", max_length=100, blank=True, null=True
    )
    mother_name = models.CharField(
        db_column="MotherName", max_length=100, blank=True, null=True
    )
    residential_village = models.CharField(
        db_column="ResidentialVillage", max_length=100, blank=True, null=True
    )
    residential_district = models.CharField(
        db_column="ResidentialDistrict", max_length=100, blank=True, null=True
    )
    residential_province = models.CharField(
        db_column="ResidentialProvince", max_length=100, blank=True, null=True
    )
    house_number = models.CharField(
        db_column="HouseNumber", max_length=100, blank=True, null=True
    )
    ...
    def get_queryset(cls, queryset, user):
        ...
        if settings.ROW_SECURITY:
            ...
            return queryset.filter(
                models.Q(
                    family__location__parent__parent_id__in=[
...
class InsureePolicy(core_models.VersionedModel):
    ...
    def get_queryset(cls, queryset, user):
        ...
        if settings.ROW_SECURITY:
            dist = UserDistrict.get_user_districts(user._u)
            return queryset.filter(
                insuree__family__location__parent__parent_id__in=[
