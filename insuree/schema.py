...
from .signals import (
    ...
    signal_before_family_query,
    signal_before_insuree_search_query,
)
    ...
    def resolve_queryset(
        ...
        qs = super(FamiliesConnectionField, cls).resolve_queryset(
            ...
            info,
            {
                k: args[k]
                for k in args.keys()
                if not k.startswith("members_") and not k.startswith("head_insuree_")
            },
            filtering_args,
            filterset_class,
        )
...
class Query(graphene.ObjectType):
    ...
    def resolve_insuree_number_validity(self, info, **kwargs):
        ...
        if errors:
            return ValidationMessageGQLType(
                False, errors[0]["errorCode"], errors[0]["message"]
            )
        else:
    ...
    def resolve_can_add_insuree(self, info, **kwargs):
        ...
        warnings = []
        policies = family.policies.filter(validity_to__isnull=True).exclude(
            status__in=[Policy.STATUS_EXPIRED, Policy.STATUS_SUSPENDED]
        )
    ...
    def resolve_insurees(self, info, **kwargs):
        ...
        if parent_location is not None:
            ...
            filters += [
                (
                    Q(current_village__isnull=False)
                    & Q(**{current_village: parent_location})
                )
                | (
                    Q(current_village__isnull=True)
                    & Q(**{family_location: parent_location})
                )
            ]

        if (
            kwargs.get("ignore_location") == False
            or kwargs.get("ignore_location") is None
        ):
            ...
            filters += [
                Q(
                    family__location__parent__parent__in=Location.objects.filter(
    ...
    def resolve_family_members(self, info, **kwargs):
        if not info.context.user.has_perms(
            InsureeConfig.gql_query_insuree_family_members
        ):
            raise PermissionDenied(_("unauthorized"))
        family = Family.objects.get(Q(uuid=kwargs.get("family_uuid")))
        return Insuree.objects.filter(
            Q(family=family), *filter_validity(**kwargs)
        ).order_by("-head", "dob")
    ...
    def resolve_families(self, info, **kwargs):
        ...
        if null_as_false_poverty is not None:
            filters += (
                [Q(poverty=True)]
                if null_as_false_poverty
                else [Q(poverty=False) | Q(poverty__isnull=True)]
            )
        show_history = kwargs.get("show_history", False)
        if not show_history:
            filters += filter_validity(**kwargs)
    ...
    def resolve_insuree_officers(self, info, **kwargs):
        if not info.context.user.has_perms(
            InsureeConfig.gql_query_insuree_officers_perms
        ):
            raise PermissionDenied(_("unauthorized"))
...
def _insuree_additional_filters(sender, additional_filter, user):
    return _get_additional_filter(
        sender, additional_filter, user, signal_before_insuree_policy_query
    )


def _insuree_insuree_additional_filters(sender, additional_filter, user):
    return _get_additional_filter(
        sender, additional_filter, user, signal_before_insuree_search_query
    )


def _family_additional_filters(sender, additional_filter, user):
    return _get_additional_filter(
        sender, additional_filter, user, signal_before_family_query
    )
